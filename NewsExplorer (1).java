 ...
 * NewsExplorer.java
 * -------------------------------------------------------------------------
 * A basic, single-file Java prototype of the News Explorer capstone:
 * keyword/topic search, automatic organization, and similarity-based
 * recommendations over a small news corpus.
 *
 * Constraint honored: no java.util.* collections (no ArrayList, HashMap,
 * List, Set, etc.) anywhere in the engine. Every data structure below
 * (dynamic array, hash map, hash set behavior) is hand-built from plain
 * arrays, matching the "students hand-build every algorithm" rule.
 * Only java.lang (String, Math, StringBuilder, Character...) and java.io
 * (for reading CLI input) are used.
 *
 * Algorithm families covered (Modules 1-3):
 *   - KMP pattern search        -> exact phrase search inside an article
 *   - Levenshtein edit distance -> fuzzy matching for misspelled queries
 *   - Inverted index            -> fast keyword -> article lookup
 *   - TF-IDF + cosine similarity-> ranked search & "related articles"
 *
 * Storage note: a real build would swap the in-memory ArticleRepository
 * below for SQLite via JDBC (java.sql.*), but that needs an external
 * sqlite-jdbc driver .jar, which can't be bundled into a single .java
 * file. The repository class is written so that swap is a drop-in change
 * (same method signatures: insert / fetchAll / fetchById).
 *
 * Compile & run:
 *   javac NewsExplorer.java
 *   java NewsExplorer demo          (scripted walkthrough)
 *   java NewsExplorer               (interactive CLI)
 * -------------------------------------------------------------------------
 */
...
public class NewsExplorer {
    ...
    static class SimpleMap<K, V> {
        ...
        private void resize() {
            ...
            for (int i = 0; i < oldCapacity; i++) {
                if (oldUsed[i]) put((K) oldKeys[i], (V) oldValues[i]);
            }
        }
    }
    ...
    static class Algorithms {
        ...
        static boolean isStopword(String w) {
            for (String s : STOPWORDS) if (s.equals(w)) return true;
            return false;
        }

        /** Manual tokenizer (no regex): lowercase words, punctuation stripped, stopwords dropped. */
            ...
            for (int i = 0; i <= text.length(); i++) {
                ...
                if (Character.isLetterOrDigit(c)) {
                    ...
                    if (current.length() > 1) {
                        ...
                        if (!isStopword(word)) tokens.add(word);
                    }
                    current.setLength(0);
                }
            }
        ...
        /** Knuth-Morris-Pratt exact pattern search. Returns match start indices. O(n + m). */
        static SimpleList<Integer> kmpSearch(String text, String pattern) {
            ...
            for (int i = 1; i < p.length(); i++) {
                while (k > 0 && p.charAt(i) != p.charAt(k)) k = failure[k - 1];
            ...
            for (int i = 0; i < t.length(); i++) {
                while (k > 0 && t.charAt(i) != p.charAt(k)) k = failure[k - 1];
        ...
        /** Returns up to topN vocabulary terms within maxDistance of queryTerm (fuzzy match). */
        static SimpleList<String> closestTerms(String queryTerm, SimpleList<String> vocabulary, int maxDistance, int topN) {
    ...
    static class SearchIndex {
        ...
        static class ScoredDoc {
            int id;
            double score;
            ScoredDoc(int id, double score) { this.id = id; this.score = score; }
        }
        ...
        void build(SimpleList<Article> articles) {
            ...
            for (int a = 0; a < articles.size(); a++) {
                ...
                for (int i = 0; i < tokens.size(); i++) {
                    ...
                    for (int p = 0; p < postings.size(); p++) {
                        if (postings.get(p) == art.id) { already = true; break; }
                    }
                    if (!already) postings.add(art.id);
                }
            ...
            for (int d = 0; d < docIds.size(); d++) {
                ...
                int totalTerms = 0;
                for (int i = 0; i < terms.size(); i++) totalTerms += counts.get(terms.get(i));
        ...
        static double cosineSimilarity(SimpleMap<String, Double> a, SimpleMap<String, Double> b) {
            ...
            for (int i = 0; i < aKeys.size(); i++) {
                ...
                if (bw != null) dot += a.get(term) * bw;
            }

            double normA = 0, normB = 0;
            for (int i = 0; i < aKeys.size(); i++) { double w = a.get(aKeys.get(i)); normA += w * w; }
            for (int i = 0; i < bKeys.size(); i++) { double w = b.get(bKeys.get(i)); normB += w * w; }
        ...
        SimpleMap<String, Double> queryVector(String query) {
            ...
            SimpleList<String> terms = counts.keySet();
            int total = 0;
            for (int i = 0; i < terms.size(); i++) total += counts.get(terms.get(i));
            if (total == 0) total = 1;
        ...
        SimpleList<ScoredDoc> rankByQuery(String query, int topN) {
            ...
            for (int i = 0; i < docIds.size(); i++) {
                ...
                if (score > 0) all.add(new ScoredDoc(docId, score));
            }
    ...
    static class ArticleRepository {
        ...
        SimpleList<Article> fetchAll() { return articles; }

        Article fetchById(int id) {
            for (int i = 0; i < articles.size(); i++) {
                if (articles.get(i).id == id) return articles.get(i);
            }
            return null;
        }
    }
    ...
    static String autoCategorize(String content) {
        ...
        for (int i = 0; i < tokens.size(); i++) tokenSet.put(tokens.get(i), true);
        ...
        for (String[] row : TOPIC_KEYWORDS) {
            ...
            for (int i = 1; i < row.length; i++) {
                if (tokenSet.containsKey(row[i])) score++;
            }
    ...
    static class NewsExplorerEngine {
        ...
        /** Article Ingestion Pipeline. Each row: {title, content, topic-or-null, source, url}. */
        void ingest(String[][] articleRows) {
            for (String[] row : articleRows) {
                String topic = row[2];
                if (topic == null || topic.isEmpty()) topic = autoCategorize(row[1]);
        ...
        /** Fuzzy suggestions ("did you mean") for query terms missing from the vocabulary. */
        SimpleList<String> fuzzySuggestions(String query) {
            ...
            for (int i = 0; i < tokens.size(); i++) {
                ...
                if (!index.vocabulary.containsKey(term)) {
                    ...
                    for (int j = 0; j < close.size(); j++) suggestions.add(term + " -> " + close.get(j));
                }
            }
            return suggestions;
        }
    ...
    static void printArticles(SimpleList<Article> articles) {
        if (articles.isEmpty()) { System.out.println("  (no results)"); return; }
        for (int i = 0; i < articles.size(); i++) {
            Article a = articles.get(i);
            System.out.printf("  #%-2d [%s] %s%n", a.id, a.topic, a.title);
        }
    }

    static void printScored(NewsExplorerEngine engine, SimpleList<SearchIndex.ScoredDoc> scored, String label) {
        if (scored.isEmpty()) { System.out.println("  (no results)"); return; }
        for (int i = 0; i < scored.size(); i++) {
            SearchIndex.ScoredDoc sd = scored.get(i);
            Article a = engine.repo.fetchById(sd.id);
            System.out.printf("  #%-2d [%s] %s  [%s: %.4f]%n", a.id, a.topic, a.title, label, sd.score);
        }
    }
    ...
    static void runDemo() {
        ...
        if (!suggestions.isEmpty()) {
            ...
            for (int i = 0; i < suggestions.size(); i++) System.out.print(suggestions.get(i) + "  ");
            System.out.println();
        }
